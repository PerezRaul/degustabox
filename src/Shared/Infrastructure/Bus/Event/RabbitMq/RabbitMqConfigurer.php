...
use Src\Shared\Domain\Bus\Event\ShouldDelay;
...
final class RabbitMqConfigurer
{
    ...
    private function queueDeclarator(
        ...
        return function (DomainEventSubscriber $subscriber) use (
            ...
            if (is_subclass_of($subscriber, ShouldDelay::class)) {
                $laterQueueName = RabbitMqQueueNameFormatter::formatLater($subscriber);
                $laterQueue     = $this->declareQueue($laterQueueName, $exchangeName, $queueName, $subscriber->delay());
                $laterQueue->bind($exchangeName, $laterQueueName);

                foreach ($subscriber::subscribedTo() as $eventClass) {
                    $laterQueue->bind($exchangeName, $eventClass::eventName());
                }
